...
extern "C" {
bool playMP3File(char* path){
  FILE *fp = fopen(path, "r");
    if (fp != NULL) {
      fclose(fp);
      int m_BCLK=26;                       // Bit Clock
      int m_LRC=25;                        // Left/Right Clock
      int m_DOUT=22;
      audio.setPinout(m_BCLK, m_LRC, m_DOUT);
      audio.setVolume(21); // 0...21
      audio.connecttoSD(path);
      float current_temp = readCurrentTemperature();
      if (current_temp > 20){ // if current temperature is above 20, then set target temp to be 10 lower than current_temperature
        setLowerTargetTemp(current_temp - 10);
      } else { // if current temperture is below 20, then set target temp to be 10 higher than curernt_temperature
        setUpperTargetTemp(current_temp + 10);
      }
      ...
      while (true){
        if (audio.isPlaying()){
          // audio has not been finished
          audio.loop();
        } else if (replay_cnt > 0){
          // audio has finished but temperature not satisfied, so start MP3 again
          audio.connecttoSD(path);
          replay_cnt --; 
        } else {
          break;
      ...
      if (target_temp_reached){
        printf("target temperature has reached, turned MP3 off\n");
        return true;
      } else {
        printf("maximum replay count reached, turned MP3 off\n");
        return false;
      }
    } else {
...
bool Audio::connecttoSD(String sdfile){
    ...
    //printf("in connect to SD\n");
    stopSong();
    //printf("after stop song\n");
    m_f_playing=false;
    m_inBuffwindex=0;
    m_inbuffrindex=0;
    ...
    if(audio_info) audio_info(chbuf);
    //printf("ID3 version=%i", m_rev);
    sprintf(chbuf,"ID3 framesSize=%i", m_id3Size);
    if(audio_info) audio_info(chbuf);
    //printf("ID3 frameSize=%i", m_id3Size);
    readID3Metadata();
    m_f_running=true;
    //free(chbuf);
    return true;
...
//    m_f_firststream_ready=false;
    ...
    tkkFunc = tkkFunc.substring(5, tkkFunc.lastIndexOf('\''));
...
void Audio::readID3Metadata(){
    ...
    if (m_f_exthdr) {
        ...
        uint8_t ehsz_bytes[4];
        fread(ehsz_bytes, 1, 4, mp3file);
        int ehsz = (ehsz_bytes[0] << 24) | (ehsz_bytes[1] << 16)
                    | (ehsz_bytes[2] << 8) | (ehsz_bytes[3]);
    ...
    do {
        ...
        if(frameid[0]==0 && frameid[1]==0 && frameid[2]==0 && frameid[3]==0){
            ...
            if(m_rev==2){
                uint8_t temp[3];
                fread(temp, 1, 3, mp3file);
                framesize = (temp[0] << 16) | (temp[1] << 8) | (temp[2]);
                ...
                uint8_t temp[4];
                fread(temp, 1, 4, mp3file);
                framesize = (temp[0] << 24) | (temp[1] << 16) | (temp[2] << 8) | (temp[3]);
                //framesize = (mp3file.read() << 24) | (mp3file.read() << 16) | (mp3file.read() << 8) | (mp3file.read());
                m_id3Size -= 4;
                //free(temp);

                uint8_t temp2[1];
                fread(temp2, 1, 1, mp3file);
                //mp3file.read(); // skip 1st flag
                m_id3Size--;
                //free(temp2);

                fread(temp2, 1, 1, mp3file);
                compressed = temp2[0] & 0x80;
            ...
            if(compressed){
                ...
                uint8_t temp[4];
                fread(temp, 1, 4, mp3file);
                int decompsize=(temp[0]<<24) | (temp[1]<<16) | (temp[2]<<8) | (temp[3]);
            ...
            if(framesize>0){
                uint8_t temp[1];
                fread(temp, 1, 1, mp3file);
                isUnicode = (temp[0]==1) ? true : false;
                //isUnicode = (mp3file.read() == 1) ? true : false;
                m_id3Size--;
                //free(temp);
                
                if(framesize<256){
                    int n = fread(value, 1, framesize-1, mp3file);
                    //mp3file.readBytes(value, framesize-1); 
                    m_id3Size-=framesize-1;
                    ...
                    if(tag=="APIC"){ // a image embedded in file, skip it
                        ...
                        fread(value, 1, 255, mp3file);
                        //mp3file.readBytes(value, 255); 
                        m_id3Size-=255;
                        value[255]=0; i=255;
                        // big block, skip it
...
void Audio::loop() {
    ...
    if(m_f_localfile){
        ...
        uint8_t tempbuf[bcw];
        res = fread(m_inBuff+m_inBuffwindex, 1, bcw, mp3file);
        //res=mp3file.read(m_inBuff + m_inBuffwindex, bcw);
        //if(res<0){} // nothing to do
        //printf("reading local file %d result: %d\n", bcw, res);
        if (res>0){
            m_inBuffwindex+=res;
            //printf("m_inBuffwindex : %d\n", m_inBuffwindex);
        }
...
uint32_t Audio::getFileSize(){
    if (!mp3file) return 0;
    fseek(mp3file, 0, SEEK_END); // seek to end of file
    uint32_t size = ftell(mp3file); // get current file pointer
    fseek(mp3file, 0, SEEK_SET); // seek back to beginning of file
    return size;
    //return mp3file.size();
}
//---------------------------------------------------------------------------------------------------------------------
...
bool Audio::setFilePos(uint32_t pos){
    ...
    return fseek(mp3file, pos, SEEK_SET);
